...
export function parseCurlCommand(curl: string): Partial<ApiRequest> {
        ...
        switch (token) {
            ...
            case "--user":
                if (tokens[i + 1]) {
                    ...
                    request.auth = { Basic: { username: username || "", password: passwordParts.join(":") || "" } };
                }
                break;
            case "-k":
            case "--insecure":
    ...
    if (!request.url && tokens.length > 1) {
        for (const t of tokens) {
            if (t !== "curl" && !t.startsWith("-") && (t.startsWith("http") || t.includes("."))) {
                request.url = t;
                break;
            }
        }
    ...
    const contentTypeHeader = Object.entries(request.headers!).find(([k]) => k.toLowerCase() === "content-type");
    ...
    if (data) {
        if (contentType.includes("application/x-www-form-urlencoded")) {
            ...
            data.split("&").forEach(pair => {
                ...
                if (key) {
                    try {
                        fields[decodeURIComponent(key.replace(/\+/g, " "))] = decodeURIComponent((val || "").replace(/\+/g, " "));
                    } catch {
                        fields[key] = val || "";
                    }
                }
            ...
            try {
                ...
                request.body = { Raw: { content: JSON.stringify(parsed, null, 2), content_type: "application/json" } };
            } catch {
                request.body = { Raw: { content: data, content_type: "application/json" } };
            }
        } else {
            request.body = { Raw: { content: data, content_type: contentType || null } };
        }
    }
