...
export function getShortcutDisplay(key: keyof typeof SHORTCUTS): ReactNode {
	return (
		<div className="flex items-center gap-0.5 font-mono text-[10px] opacity-50">
			{isMac ? (
				SHORTCUTS[key].mac.split("+").map((part) => {
					if (part === "Cmd") return <BsCommand key="mac-cmd" />;
...
export function getSimpleShortcut(
	action: "Cut" | "Copy" | "Paste" | "Delete" | "Duplicate",
): ReactNode {
	const map = {
		Cut: SHORTCUTS.CUT,
		Copy: SHORTCUTS.COPY,
