...
function parseInsomniaWebSocket(
	resource: InsomniaWebSocketRequest,
): WebSocketFile {
...
export function parseInsomniaExport(data: any): Partial<Project> {
	...
	function buildFolder(parentId: string, name: string): Folder {
		...
		for (const child of childResources) {
			if (child._type === "request_group") {
				...
				children.push(
					parseInsomniaWebSocket(child as InsomniaWebSocketRequest),
				);
			}
		}
	...
	const baseEnv = resources.find(
		(r) => r._type === "environment" && r.parentId === workspace._id,
	) as InsomniaEnvironment | undefined;

	if (baseEnv) {
		const subEnvs = resources.filter(
			(r) => r._type === "environment" && r.parentId === baseEnv._id,
