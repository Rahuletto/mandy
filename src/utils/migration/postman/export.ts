...
import type { Folder, Project, RequestFile } from "../../../types/project";
	...
	if ("ApiKey" in auth) {
		return {
			...
			apikey: [
				...
				{
					key: "in",
					value: auth.ApiKey.add_to === "Header" ? "header" : "query",
					type: "string",
				},
			],
		};
...
function convertBodyToPostman(body: BodyType): PostmanBody | null {
	...
	if ("FormUrlEncoded" in body) {
		return {
			mode: "urlencoded",
			urlencoded: Object.entries(body.FormUrlEncoded.fields).map(
				([key, value]) => ({
					key,
					value: value || "",
				}),
...
function convertRequestToPostman(req: RequestFile): PostmanItem {
	...
	const headers: PostmanHeader[] = Object.entries(apiReq.headers).map(
		([key, value]) => ({
			key,
			value: value || "",
		}),
