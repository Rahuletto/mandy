...
function getNestedValue(
	...
	if (context.lastResponse) {
		...
		switch (root) {
			...
			default: {
				const lr = context.lastResponse;
				if (lr && typeof lr === "object" && Object.hasOwn(lr, root)) {
					value = (lr as Record<string, unknown>)[root];
				} else {
...
export class WorkflowEngine {
	...
	async run(): Promise<void> {
		...
		const initialVariables = startData.variables || {};

		const envVars = startData.envVariables || [];
		...
		try {
			...
			switch (nodeData.type) {
				...
				case "loop": {
					...
					const collect = !!loopData.collectResults;
	...
	private resolveConditionEdge(
		...
		const sorted = [...outgoingEdges].sort(
			(a, b) => scoreEdge(b) - scoreEdge(a),
		);
...
export async function testLoopCollectionExample(): Promise<{
	...
	const nodes: Node<WorkflowNodeData>[] = [
		{
			id: "start",
			position: { x: 0, y: 0 },
			data: { type: "start", label: "Start", status: "idle" },
		},
		{
			id: "loop1",
			position: { x: 0, y: 0 },
		...
		},
		{
			id: "end",
			position: { x: 0, y: 0 },
			data: { type: "end", label: "End", status: "idle" },
		},
	];
	...
	const edges: Edge[] = [
		...
		{ id: "e3", source: "loop1", sourceHandle: "exit", target: "end" },
	];

	const loopOutputRef = { current: undefined as NodeOutput | undefined };
	...
	const engine = new WorkflowEngine(
		...
		() => {}, // onNodeStatusChange
		() => {}, // onEdgeStatusChange
		async (_requestId, ctx: WorkflowExecutionContext) => ({
			status: 200,
			body: { i: ctx.loopIndex },
		}), // executeRequest
		async () => false, // evaluateCondition (unused)
		(nodeId, output) => {
			if (nodeId === "loop1") loopOutputRef.current = output;
		},
	);
	...
	try {
		await engine.run();
		let success = false;
		const loopOut = loopOutputRef.current;
		if (loopOut) {
			const bodiesOk = Array.isArray(loopOut.body) && loopOut.body.length === 3;
			const outputsOk =
				Array.isArray(loopOut.iterationOutputs) &&
				loopOut.iterationOutputs.length === 3;
