...
import type { ApiResponse, AuthType } from "../bindings";
import {
...
function scheduleRealtimeFullTeardown() {
	void import("../realtime/globalRealtimeBridge").then((m) =>
		m.shutdownAllRealtimeTransports(),
	);
}

function scheduleRealtimeTeardownForItem(itemId: string) {
	void import("../realtime/globalRealtimeBridge").then((m) =>
		m.teardownRealtimeForConnection(itemId),
	);
}
...
export const useProjectStore = create<ProjectState>()(
	persist(
		(set, get) => ({
			...
			addItem: (type, parentFolderId, name) => {
				...
				set((state) => ({
					...
					projects: state.projects.map((p) => {
						...
						return {
							...p,
							root: addChildToFolder(p.root, parentFolderId, item),
						};
					}),
					activeItemId: newId,
				}));
			...
			deleteItem: (itemId) => {
				scheduleRealtimeTeardownForItem(itemId);
				set((state) => {
					const project = state.projects.find(
						(p) => p.id === state.activeProjectId,
					);
		...
		}),
		{
			...
			migrate: (persistedState, fromVersion) => {
				if (fromVersion < ZUSTAND_PERSIST_VERSION) {
					return migratePersistedZustandState(persistedState, fromVersion) as {
						projects: Project[];
						activeProjectId: string | null;
						activeItemId: string | null;
						selectedItemId: string | null;
