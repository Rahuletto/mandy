...
export function ProtocolToggle({ value, onChange }: ProtocolToggleProps) {
  ...
  return (
    ...
    >
      <span
        className={`text-[10px] font-medium transition-colors ${isTcp ? "text-white/70" : "text-white/30"}`}
      >
        TCP
      </span>
      <button
      ...
      <span
        className={`text-[10px] font-medium transition-colors ${!isTcp ? "text-accent" : "text-white/30"}`}
      >
        QUIC
      </span>
