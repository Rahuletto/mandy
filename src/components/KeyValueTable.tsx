...
export function KeyValueTable({
	...
	return (
		<div className="flex flex-col h-full">
			{title ? (
				<h3 className="mb-2 px-1 text-xs font-semibold text-white/60">
					{title}
				</h3>
			) : null}
			<div className="flex-1 overflow-x-auto overflow-y-auto">
				<table className="w-full min-w-full text-xs">
					<thead className="sticky top-0 z-10">
						<tr className="border-b border-white/10">
							{!readOnly && (
								<th className="w-10 px-3 py-2.5 border-r border-white/10"></th>
							)}
							<th className="text-left px-3 py-2.5 font-medium text-white/40 min-w-[150px] border-r border-white/10">
								Key
							</th>
							<th
								className={`text-left px-3 py-2.5 font-medium text-white/40 min-w-[200px] ${showDescription ? "border-r border-white/10" : ""}`}
							>
								Value
							</th>
							{showDescription && (
								<th className="text-left px-3 py-2.5 font-medium text-white/40 min-w-[150px]">
									Description
								</th>
							)}
                ...
                `}
                      ...
                      `}
								...
								<td
									className={`px-3 py-2 ${showDescription ? "border-r border-white/5" : ""}`}
								>
									{readOnly || item.locked ? (
									...
									<td className="px-3 py-2 text-center">
										{item.key !== undefined && !item.locked && (
											<button
												...
												className="opacity-0 group-hover:opacity-100 w-5 h-5 flex items-center justify-center rounded text-white/30 hover:text-red hover:bg-red/10 transition-all mx-auto"
											>
												<svg
													width="12"
													height="12"
