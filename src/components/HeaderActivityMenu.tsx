...
export function HeaderActivityMenu({
	...
	return (
		<>
			...
			<div
				ref={anchorRef}
				className="no-drag relative inline-flex flex-col items-center gap-2"
			...
			>
				...
				<HoverPopover
					...
					onMouseLeave={scheduleClose}
					position="bottom"
					className="no-drag !p-2 z-[200] min-w-62.5 max-w-[min(360px,calc(100vw-24px))]"
				>
					<div className="flex max-h-56 flex-col gap-1 overflow-auto">
						{rows.map((row) => (
							...
							>
								...
								<button
									type="button"
									className="min-w-0 flex-1 cursor-pointer truncate text-left text-white/90 text-xs hover:text-white"
									onClick={() => {
										const id =
