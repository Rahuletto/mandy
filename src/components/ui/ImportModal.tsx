...
type ImportSource = "matchstick" | "openapi" | "openapi-file" | "openapi-url" | "curl" | "postman" | "insomnia" | "hoppscotch" | "gist";
                ...
                {!selectedSource ? (
                    <div className="p-2 space-y-1">
                        {importSources.map((source) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <TbChevronRight size={14} className="text-white/20 group-hover:text-white/50" />
                                        </div>
                                        ...
                                        {showOpenAPIMenu && (
                                            <div className="absolute right-0 top-0 translate-x-full pl-2 z-[60]">
                                                <div className="min-w-[160px] bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-left-2 duration-200 p-2 space-y-1">
                                                    ...
                                                    >
                                                        <TbFileDescription size={18} className="text-white/40 group-hover/item:text-white/80 transition-colors" />
                                                    ...
                                                    <button
                                                        ...
                                                        className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs text-white/70 hover:text-white hover:bg-white/10 transition-colors text-left group/item"
                                                    >
                                                        <TbLink size={18} className="text-white/40 group-hover/item:text-white/80 transition-colors" />
                                                        <div className="flex flex-col">
                            ...
                            return (
                                ...
                                >
                                    <Icon size={16} className={`${source.available ? "text-white/40 group-hover:text-white/80" : "text-white/20"} transition-colors`} />
                                    <span className={`flex-1 text-sm ${source.available ? "text-white/70 group-hover:text-white" : "text-white/30"} transition-colors`}>
                                        {source.label}
                                    </span>
                                </button>
                            );
                    ...
                    <div className="p-4 space-y-4">
                        {selectedSource === "openapi-url" ? (
                            <div>
                                ...
                                <div className="relative group">
                                    <TbLink className="absolute left-3 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-white/50 transition-colors" size={14} />
                                    <input
                                        type="url"
                                        autoFocus
                                        value={urlInput}
                            ...
                            <div>
                                ...
                                >
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${fileContent ? "bg-accent/20" : "bg-white/5 group-hover:bg-white/10"
                                        }`}>
                                        <TbUpload size={16} className={fileContent ? "text-accent" : "text-white/40"} />
                                    </div>
                        ...
                        <button
                            ...
                            disabled={loading || (selectedSource === "openapi-url" ? !urlInput : !fileContent)}
