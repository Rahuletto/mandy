            ...
            <div className={`flex gap-1 border-b border-white/10 ${className}`}>
                {tabs.map((tab) => (
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium transition-colors cursor-pointer relative ${activeTab === tab.id
                            ? "text-accent"
        ...
        <div className={`flex gap-1 ${className}`}>
            {tabs.map((tab) => (
                <button
                    ...
                    type="button"
                    onClick={() => onTabChange(tab.id)}
                    className={`${sizeClasses} cursor-pointer font-medium rounded-md transition-colors ${activeTab === tab.id
