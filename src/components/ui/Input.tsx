...
export function EnvInput({
	...
	return (
		<div
			className={`relative flex flex-1 items-center ${className} ${disabled ? "cursor-not-allowed opacity-60" : ""}`}
		>
			<div
				ref={editorRef}
				...
				className={`relative z-10 flex-1 whitespace-pre px-3 text-white/80 text-xs leading-normal focus:outline-none ${disabled ? "pointer-events-none" : ""}`}
				suppressContentEditableWarning
			/>
			{!value && !isFocused && (
				<div className="pointer-events-none absolute left-3 select-none text-white/20 text-xs">
					{placeholder}
				</div>
			)}
			...
			{isPassword && value && (
				<button
					...
					disabled={disabled}
					className={`z-20 ml-2 flex-shrink-0 p-0.5 text-white/30 transition-colors ${disabled ? "" : "hover:text-white/60"}`}
				>
...
export function UrlInput({
	...
	const handleInvalidSpanEnter = useCallback((e: MouseEvent) => {
	...
	const handleInvalidSpanLeave = useCallback(() => {
	...
	}, [handleInvalidSpanLeave, handleInvalidSpanEnter]);
	...
	return (
		<div className={`relative flex flex-1 items-center ${className}`}>
			<div
				...
				onBlur={handleBlur}
				onKeyDown={handleKeyDown}
				className={`relative z-10 w-full whitespace-pre px-3 py-2.5 text-sm text-white/90 focus:outline-none ${disabled ? "pointer-events-none" : ""} ${hasInvalidVariables ? "pr-8" : ""}`}
				suppressContentEditableWarning
			/>
			{!value && !isFocused && (
				<div className="pointer-events-none absolute left-3 select-none text-sm text-white/20">
					{placeholder}
				</div>
			)}
			{hasInvalidVariables && (
				<>
					...
					<div
						ref={warningRef}
						className="absolute top-1/2 right-3 z-20 -translate-y-1/2 cursor-help text-red"
					...
					<HoverPopover
						...
						onMouseEnter={handleWarningEnter}
						onMouseLeave={handleWarningLeave}
						className="!px-2 !py-1 !rounded-lg bg-card text-white text-xs"
					>
					...
					{showInvalidPopover && (
						<div
							ref={invalidSpanRef}
							className="fixed z-50 min-w-[200px] max-w-[280px] animate-blur-in rounded-xl border border-border bg-card p-3 shadow-2xl"
							style={{ top: popoverPosition.top, left: popoverPosition.left }}
							onMouseEnter={handlePopoverEnter}
							onMouseLeave={handlePopoverLeave}
						>
							<div className="flex flex-col gap-2">
								<div className="font-medium text-[11px] text-red">
								...
								<div className="flex flex-wrap gap-1">
									{invalidVariableNames.map((varName) => (
										<span
											key={varName}
											className="rounded bg-red/20 px-1.5 py-0.5 font-mono text-[11px] text-red"
										>
											{`{{${varName}}}`}
										</span>
									))}
								...
								{availableVariables.length > 0 && (
									<>
										...
										<div className="font-medium text-[11px] text-white/50">
											Available variables
										</div>
										<div className="flex flex-wrap gap-1">
											{availableVariables.map((varName) => (
												<button
													...
													className={`rounded bg-accent/20 px-1.5 py-0.5 font-mono text-[11px] text-accent transition-colors ${invalidVariableNames.length === 1 ? "cursor-pointer hover:bg-accent/30" : "cursor-default"}`}
													title={
