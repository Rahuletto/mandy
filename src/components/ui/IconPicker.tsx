...
export function IconPicker({
    ...
    return (
        <div
            ref={popoverRef}
            className={`fixed z-50 bg-[#1e1e1e] border border-white/10 rounded-xl shadow-2xl p-3 w-[216px] ${isAnimatingOut ? "animate-blur-out" : "animate-blur-in"
                }`}
            style={{
                left: position.x,
                top: position.y,
            ...
            <div className="grid grid-cols-6 gap-1">
                {ICON_OPTIONS.map(({ icon: Icon, name }) => (
                    <button
                        ...
                        className={`w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 transition-colors cursor-pointer group ${selectedIcon === name ? "ring-1 ring-white/20" : ""
                            }`}
                        style={{ backgroundColor: selectedIcon === name ? (selectedColor ? hexToRgba(selectedColor, 0.15) : "rgba(255, 255, 255, 0.1)") : undefined }}
                    >
                        <Icon
                            size={16}
            ...
            <div className="grid grid-cols-7 gap-1.5 px-0.5">
                {COLOR_OPTIONS.map(({ name, value }) => (
                    <button
                        ...
                        className={`w-5 h-5 rounded-full transition-transform hover:scale-110 cursor-pointer relative flex items-center justify-center ${selectedColor === value ? "ring-2 ring-white/40 ring-offset-2 ring-offset-[#1e1e1e]" : ""
                            }`}
