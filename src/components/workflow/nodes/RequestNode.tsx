...
export const RequestNode = memo(function RequestNode({ id, data }: NodeProps) {
	...
	return (
		<div
			className={`min-w-[160px] px-3 py-2 rounded-lg border-2 transition-colors backdrop-blur-md ${getNodeStyles(status)}`}
		>
			...
			<div className="flex items-center gap-2">
				...
				{status === "running" && (
					<VscLoading className="text-accent animate-spin ml-auto" size={14} />
				)}
				{status === "completed" && (
					<VscCheck className="text-green ml-auto shrink-0" size={14} />
				)}
				{status === "error" && (
					<VscClose className="text-red ml-auto shrink-0" size={14} />
				)}
			</div>
			{metricsLine ? (
				<div
					className="mt-1 text-[10px] font-mono text-white/45 tabular-nums truncate max-w-[200px]"
					title={metricsLine}
				>
					{metricsLine}
				</div>
