...
function WorkflowEditorInner({
  ...
  const [requestPopover, setRequestPopover] = useState<{ x: number; y: number } | null>(null);
  const [nodeTypePopover, setNodeTypePopover] = useState<{ x: number; y: number } | null>(null);
    ...
    return edges.map((edge) => {
      ...
      if (errorEdges.has(edge.id) || sourceStatus === "error") {
        return {
          ...
          animated: false,
          markerEnd: { type: MarkerType.ArrowClosed, width: 16, height: 16, color: "var(--red)" },
        };
      }
      ...
      if (flashingEdges.has(edge.id) || _flashCount > 0) {
        ...
        return {
          ...
          animated: true,
          markerEnd: { type: MarkerType.ArrowClosed, width: 16, height: 16, color: _flashColor },
        };
      }
      ...
      if (
        ...
        return {
          ...
          animated: false,
          markerEnd: { type: MarkerType.ArrowClosed, width: 16, height: 16, color: "var(--green)" },
        };
      }
  ...
  }, [edges, nodes, runningEdges, completedEdges, errorEdges, flashingEdges, flashCounters, loopBodyEdges]);
  ...
  const updateNodeData = useCallback(
    (nodeId: string, partialData: Partial<WorkflowNodeData>) => {
      setNodes((nds) =>
        nds.map((n) =>
          n.id === nodeId
            ? { ...n, data: { ...(n.data as WorkflowNodeData), ...(partialData as any) } }
            : n,
        ),
      );
    },
  ...
  const updateEdgeStatus = useCallback(
    (edgeId: string, status: "idle" | "running" | "completed" | "error") => {
      if (status === "running") {
        ...
        setRunningEdges((prev) => { const next = new Set(prev); next.delete(edgeId); return next; });
  ...
  const resetNodeStatuses = useCallback(() => {
    setNodes((nds) =>
      nds.map((node) => ({
        ...node,
        data: { ...(node.data as WorkflowNodeData), status: "idle" as WorkflowNodeStatus },
      })),
    );
    ...
    (nodeId: string, updates: Partial<WorkflowNodeData>) => {
      ...
      setNodes((nds) =>
        nds.map((node) => {
          ...
          return { ...(node as Node<WorkflowNodeData>), data } as Node<WorkflowNodeData>;
        }),
      );
    },
  ...
  const getAvailableOutputs = useCallback(() => {
    ...
    if ((selectedNode.data as WorkflowNodeData).type === "end") {
      ...
      for (const sourceNode of nodes) {
        ...
        const outputData: (typeof outputs)[number] = {
        ...
        if (nodeData.type === "request") {
          ...
          outputData.nodeName = requestData.requestName || nodeData.label || sourceNode.id;
  ...
  const inferLoopBodyEdgeIds = useCallback(
    (allEdges: Edge[], allNodes: Node<WorkflowNodeData>[]): Set<string> => {
      ...
      const loopNodes = allNodes.filter((n) => (n.data as WorkflowNodeData).type === "loop");

      for (const loopNode of loopNodes) {
        const outgoing = allEdges.filter((e) => e.source === loopNode.id);
        if (outgoing.length === 0) continue;
        if (outgoing.length === 1) { loopBodyEdgeIds.add(outgoing[0].id); continue; }

        const getTargetNode = (edge: Edge) => allNodes.find((n) => n.id === edge.target);
        ...
        const loopBody = bestBody || [...outgoing].sort((a, b) => bodyScore(b) - bodyScore(a))[0];
  ...
  const handleRunWorkflow = useCallback(async () => {
    if (isRunning) {
      if (isStopping) { forceKillWorkflow(); return; }
      setIsStopping(true);
      workflowEngineRef.current?.stop();
      haptic("alignment");
      stopTimeoutRef.current = setTimeout(() => {
        addToast("Stopping takes longer than usual. Click Stop again to force kill.", "warning");
      }, 3000);
      return;
    }
    ...
    if (hasCycle) {
      addToast("Workflow contains a directed cycle which may cause infinite execution. Remove cycles before running.", "warning");
      return;
    }
    ...
    const engine = new WorkflowEngine(
      ...
      async (requestId: string, context: any, overrides?: RequestOverrides) => {
        if (forceKilledRef.current) throw new Error("Workflow was force killed");
        if (onExecuteRequest) {
      ...
      (edgeId: string, opts?: { color?: string; ms?: number }) => {
        ...
        setFlashCounters((prev) => { const next = { ...prev }; next[edgeId] = (next[edgeId] || 0) + 1; return next; });
        setFlashingEdges((prev) => new Set(prev).add(edgeId));
        setEdges((prev) =>
          prev.map((e) =>
            e.id === edgeId
              ? { ...e, data: { ...(e.data || {}), __flash: Date.now(), __flashColor: color, __flashMs: flashMs } }
              : e,
          ),
        );
        setTimeout(() => {
          setFlashingEdges((prev) => { const next = new Set(prev); next.delete(edgeId); return next; });
          setFlashCounters((prev) => { const next = { ...prev }; delete next[edgeId]; return next; });
          setEdges((prev) =>
            prev.map((e) =>
              e.id === edgeId
                ? { ...e, data: { ...(e.data || {}), __flash: null, __flashColor: null, __flashMs: null } }
                : e,
            ),
          );
        }, flashMs);
      ...
      (nodeId: string, opts?: { ms?: number }) => {
        ...
        setNodeFlashCounters((prev) => { const next = { ...prev }; next[nodeId] = (next[nodeId] || 0) + 1; return next; });
        setNodes((prev) =>
          prev.map((n) =>
            n.id === nodeId
              ? { ...n, data: { ...(n.data as WorkflowNodeData), __flashCount: ((n.data as any).__flashCount || 0) + 1 } }
              : n,
          ),
        );
        setTimeout(() => {
          setNodeFlashCounters((prev) => { const next = { ...prev }; delete next[nodeId]; return next; });
          setNodes((prev) =>
            prev.map((n) =>
              n.id === nodeId
                ? { ...n, data: { ...(n.data as WorkflowNodeData), __flashCount: 0 } }
                : n,
            ),
          );
        }, flashMs);
    ...
    try {
      ...
      setTimeout(() => {
        const endNode = nodes.find((n) => (n.data as WorkflowNodeData).type === "end");
    ...
    nodes, edges, isRunning, isStopping, onRunWorkflow, onExecuteRequest,
    updateNodeStatus, updateEdgeStatus, resetNodeStatuses, addToast,
    updateNodeData, forceKillWorkflow, inferLoopBodyEdgeIds,
  ]);
    ...
    (params: Connection) => {
      ...
      setEdges((eds) => {
        ...
        while (stack.length > 0) {
          ...
          if (nodeId === params.source) { createsCycle = true; break; }
          if (visited.has(nodeId)) continue;
          visited.add(nodeId);
          for (const n of adjacency[nodeId] || []) { if (!visited.has(n)) stack.push(n); }
        }
        ...
        if (createsCycle) {
          ...
          const loopInCycle =
            ...
            !!nodes.find((n) => n.id === params.source && (n.data as WorkflowNodeData).type === "loop") ||
            !!nodes.find((n) => n.id === params.target && (n.data as WorkflowNodeData).type === "loop");
          if (!loopInCycle) {
  ...
  const addConditionNode = useCallback(() => {
    ...
    setNodes((nds) => [
      ...nds,
      {
        ...
        type: "condition",
        position: center,
        data: { type: "condition", label: "New Condition", status: "idle", expression: "return status == 200" },
  ...
  const addLoopNode = useCallback(() => {
    ...
    setNodes((nds) => [
      ...nds,
      {
        ...
        type: "loop",
        position: center,
        data: { type: "loop", label: "New Loop", status: "idle", loopType: "count", iterations: 3, delayMs: 0 },
      } as Node<WorkflowNodeData>,
    ]);
  }, [setNodes, beginUserAction, getViewportCenter]);
    ...
    <div className="flex flex-col h-full bg-background">
        ...
        {activeTab === "editor" && (
          <button
            ...
            className={`flex items-center rounded-full px-6 py-2 text-sm font-semibold text-card transition-colors ${
              isRunning
                ? isStopping ? "bg-yellow hover:bg-red" : "bg-red hover:bg-red"
        ...
        {(["overview", "editor"] as const).map((tab) => (
          <button
            ...
            className={`px-2 py-0.5 text-xs cursor-pointer font-medium rounded-md transition-colors ${
              activeTab === tab ? "text-accent bg-accent/10" : "text-white/80 hover:text-white/60"
            }`}
          >
      ...
      {activeTab === "overview" ? (
        <WorkflowOverview
          ...
          onRun={() => { setActiveTab("editor"); handleRunWorkflow(); }}
          isRunning={isRunning}
        ...
        <>
          <div className="flex-1 flex overflow-hidden relative">
            <div ref={reactFlowWrapper} className="flex-1 relative">
              ...
              >
                <Background variant={BackgroundVariant.Dots} gap={20} size={3} color="rgba(255,255,255,0.1)" />
              ...
              onAddRequest={(requestId, requestName, method) => {
                ...
                setNodes((nds) => [
                  ...nds,
                  {
                    ...
                    type: "request",
                    position: center,
                    data: { type: "request", label: requestName, status: "idle", requestId, requestName, method },
                  } as Node<WorkflowNodeData>,
                ]);
              }}
            />
