...
import { generateCurl, generateFetch, generatePythonRequests, generateGo, generateRust, generateJava, generatePHP } from "../utils/snippets";
import { CodeViewer } from "./CodeMirror";
import { Dropdown } from "./ui";
import { decodeBody } from "../reqhelpers/rest";
import { ObjectDefinition } from "../types/overview";
import { getTypeColor, extractDefinitions, scrollToId } from "../utils/overviewUtils";
...
export const RequestOverview: React.FC<RequestOverviewProps> = ({
    ...
    const getSnippet = () => {
        switch (snippetLang) {
            case "Shell cURL": return { code: generateCurl(activeRequest.request), lang: "shell" as const };
            case "JavaScript Fetch": return { code: generateFetch(activeRequest.request), lang: "javascript" as const };
            case "Python Requests": return { code: generatePythonRequests(activeRequest.request), lang: "python" as const };
            case "Go Native": return { code: generateGo(activeRequest.request), lang: "go" as const };
            case "Rust Reqwest": return { code: generateRust(activeRequest.request), lang: "rust" as const };
            case "Java HttpClient": return { code: generateJava(activeRequest.request), lang: "java" as const };
            case "PHP Guzzle": return { code: generatePHP(activeRequest.request), lang: "php" as const };
            default: return { code: "", lang: "text" as const };
        }
    };
    ...
    const definitions = useMemo(() => {
        ...
        if (reqBody !== "None" && "Raw" in reqBody && reqBody.Raw.content_type?.includes("json")) {
            try {
                const data = JSON.parse(reqBody.Raw.content);
                allDefs = [...allDefs, ...extractDefinitions(data, 'RequestBody', seen)];
            } catch (e) { }
        }
        ...
        if (activeRequest.response) {
            ...
            if (bodyText) {
                try {
                    ...
                    allDefs = [...allDefs, ...extractDefinitions(data, 'ResponseBody', seen)];
                } catch (e) { }
            }
        }
    ...
    const renderProperty = (key: string, value: any, context?: string, allowDescription?: boolean, showTypes: boolean = true) => {
        ...
        let targetId = "";
        if (isObject) targetId = `def-${key.charAt(0).toUpperCase() + key.slice(1)}`;
        if (isObjectArray) targetId = `def-${key.charAt(0).toUpperCase() + key.slice(1)}Item`;
                    ...
                    <div className="mt-1 flex items-center min-h-[18px]">
                        {editingProperty === fullKey ? (
                            <input
                                ...
                                onKeyDown={(e) => e.key === "Enter" && handlePropDescBlur(fullKey)}
    ...
    const renderStructure = (title: string, data: any, showSwitch?: boolean, context?: string, allowDescription?: boolean) => {
        if (!data) return null;
        return (
            <div className="mt-4">
                        ...
                        <section className="flex flex-col gap-8">
                            ...
                            {(() => {
                                ...
                                if (body !== "None" && "Raw" in body && body.Raw.content_type?.includes("json")) {
                                    try { return renderStructure("Request Body", JSON.parse(body.Raw.content), true, "request", true); } catch (e) { }
                                }
                                return null;
                            })()}
                            ...
                            {(() => {
                                ...
                                try { return renderStructure("Response Body", JSON.parse(bodyText), false, "response", false); } catch (e) { }
                                return null;
                            })()}

                            {definitions.length > 0 && (
                                <div className="mt-8">
                                    <h3 className="text-sm font-semibold text-white/70 mb-4">Object Definitions</h3>
                                    <div className="space-y-4">
                                        {definitions.map((def) => (
                                            <div key={def.name} id={`def-${def.name}`} className="scroll-mt-12 transition-colors duration-500 rounded-lg">
                    ...
                    <div className="h-full rounded-xl bg-background border border-white/5 overflow-hidden flex flex-col shadow-2xl">
                        <div className="flex shrink-0 items-center justify-between px-4 py-2 bg-white/5 border-b border-white/5">
                            <div className="flex items-center gap-2">
                                <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${activeRequest.request.method === 'GET' ? 'bg-green-500/20 text-green-400' :
                                    activeRequest.request.method === 'POST' ? 'bg-blue-500/20 text-blue-400' :
                                        activeRequest.request.method === 'PUT' ? 'bg-yellow-500/20 text-yellow-400' :
                                            activeRequest.request.method === 'DELETE' ? 'bg-red-500/20 text-red-400' :
                            ...
                            <div className="relative">
                                ...
                                {showSnippetDropdown && (
                                    <Dropdown
                                        ...
                                        items={[
                                            { label: "Shell cURL", onClick: () => { setSnippetLang("Shell cURL"); setShowSnippetDropdown(false); } },
                                            { label: "JavaScript Fetch", onClick: () => { setSnippetLang("JavaScript Fetch"); setShowSnippetDropdown(false); } },
                                            { label: "Python Requests", onClick: () => { setSnippetLang("Python Requests"); setShowSnippetDropdown(false); } },
                                            { label: "Go Native", onClick: () => { setSnippetLang("Go Native"); setShowSnippetDropdown(false); } },
                                            { label: "Rust Reqwest", onClick: () => { setSnippetLang("Rust Reqwest"); setShowSnippetDropdown(false); } },
                                            { label: "Java HttpClient", onClick: () => { setSnippetLang("Java HttpClient"); setShowSnippetDropdown(false); } },
                                            { label: "PHP Guzzle", onClick: () => { setSnippetLang("PHP Guzzle"); setShowSnippetDropdown(false); } },
                                        ]}
                                    />
                                )}
                            </div>
