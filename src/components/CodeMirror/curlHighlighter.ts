...
function getCurlDecorations(view: EditorView) {
    ...
    while ((match = headerRegex.exec(text)) !== null) {
        ...
        if (valueStart < valueEnd) builder.add(valueStart, valueEnd, valueDecoration);
    }
    ...
    while ((match = jsonBlockRegex.exec(text)) !== null) {
        ...
        while ((innerMatch = innerValueRegex.exec(bodyWithQuotes)) !== null) {
            ...
            const start = bodyStart + innerMatch.index + startOffset + (isString ? 1 : 0);
...
export const curlHighlighter = ViewPlugin.fromClass(class {
