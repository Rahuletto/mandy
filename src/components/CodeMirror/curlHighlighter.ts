...
function getCurlDecorations(view: EditorView) {
	...
	while (match !== null) {
		...
		let innerMatch: RegExpExecArray | null = innerKeyRegex.exec(bodyWithQuotes);
		while (innerMatch !== null) {
			...
			innerMatch = innerKeyRegex.exec(bodyWithQuotes);
		}

		const innerValueRegex = /:\s*("([^"]*)"|(-?\d+\.?\d*)|true|false|null)/g;
		innerMatch = innerValueRegex.exec(bodyWithQuotes);
		while (innerMatch !== null) {
