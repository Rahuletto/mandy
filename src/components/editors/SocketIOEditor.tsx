...
export function SocketIOEditor({
  ...
  const connect = useCallback(async () => {
    ...
    const resolvedUrl = buildSocketIoUrl(url, sio.path, sio.queryItems, resolve);
        ...
        startTransition(() => {
          onUpdate((prev) => ({
            ...
            messages: appendSioMessage(prev.messages, {
              ...
              event: ackResult.timed_out ? "ack-timeout" : `${eventName.trim()}:ack`,
  ...
  const headerConnectTooltip = !url
    ...
    : isConnecting
      ? "Connecting..."
      : undefined;
  const connectionSectionClass = "space-y-4 border-b border-white/6 pb-4 last:border-b-0";
  const fieldClass =
    ...
    <div className="flex flex-col h-full">
      <div className="flex gap-4 border-b border-text/15 p-4">
        <div className="flex-1 flex items-center bg-inputbox rounded-lg overflow-hidden">
          <span className="px-4 py-2.5 flex items-center text-[#25C2A0]">
            <SiSocketdotio size={18} className="text-[#25C2A0]" aria-hidden />
          </span>
      ...
      <div ref={splitContainerRef} className="flex-1 min-h-0 flex overflow-hidden">
        ...
        >
          <div className="flex items-center gap-1 py-2 shrink-0">
            {(["overview", "emit", "headers", "authorization", "connection"] as const).map((tab) => (
              <button
                key={tab}
                type="button"
                  ...
                  <div className="mt-3 flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        step={100}
                        value={ackTimeoutMs}
                        onChange={(e) => setAckTimeoutMs(Math.max(100, Number(e.target.value) || 3000))}
                        disabled={!awaitAck}
              ...
              <div className="space-y-4 p-4">
                <section className={connectionSectionClass}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="mb-2 block text-xs text-white/60">Namespace</label>
                      <input
                        type="text"
                        value={sio.namespace || "/"}
                        onChange={(e) =>
                          onUpdate((prev) => ({ ...prev, namespace: e.target.value || "/" }))
                        }
                    ...
                    <div>
                      <label className="mb-2 block text-xs text-white/60">Path</label>
                      <input
                        type="text"
                        value={sio.path || "/socket.io/"}
                        onChange={(e) =>
                          onUpdate((prev) => ({ ...prev, path: e.target.value }))
                        }
                        className={`${fieldClass} font-mono`}
                  ...
                  <div className="space-y-2">
                    <label className="block text-xs text-white/60">Transport</label>
                ...
                <section className={connectionSectionClass}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <label className="inline-flex min-h-10 items-center gap-2 text-sm font-medium text-white/72">
                      <Checkbox
                        ...
                        onChange={(checked) =>
                          onUpdate((prev) => ({ ...prev, reconnectOnDisconnect: checked }))
                        }
                      />
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="mb-2 block text-xs text-white/60">Reconnect Min (ms)</label>
                      <input
                        type="number"
                        min={0}
                        ...
                        onChange={(e) =>
                          onUpdate((prev) => ({
                            ...prev,
                            reconnectDelayMinMs: Math.max(0, Number(e.target.value) || 0),
                          }))
                        }
                    ...
                    <div>
                      <label className="mb-2 block text-xs text-white/60">Reconnect Max (ms)</label>
                      <input
                        type="number"
                        min={0}
                        ...
                        onChange={(e) =>
                          onUpdate((prev) => ({
                            ...prev,
                            reconnectDelayMaxMs: Math.max(0, Number(e.target.value) || 0),
                          }))
                        }
                        className={fieldClass}
                        disabled={isConnected}
                    ...
                    <div>
                      <label className="mb-2 block text-xs text-white/60">Max Attempts</label>
                      <input
                        type="number"
                        min={0}
                        ...
                        onChange={(e) =>
                          onUpdate((prev) => ({
                            ...prev,
                            maxReconnectAttempts: Math.max(0, Number(e.target.value) || 0),
                          }))
                        }
                        className={fieldClass}
                        disabled={isConnected}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        onUpdate((prev) => ({
                          ...prev,
                          queryItems: [
                            ...(prev.queryItems || []).filter((item) => item.key !== nextKey),
                            {
                              id: crypto.randomUUID(),
                              key: nextKey,
                              value: queryDraftValue,
                        ...
                        >
                          <input
                            ...
                            onChange={(e) =>
                              onUpdate((prev) => ({
                                ...prev,
                                queryItems: (prev.queryItems || []).map((entry) =>
                                  entry.id === item.id
