...
import type { MQTTFile, MQTTMessage, MQTTSubscription } from "../../types/project";
import { CodeEditor } from "../CodeMirror";
import { AutocompleteInput, Checkbox, Dropdown, Tooltip } from "../ui";
import { MQTTOverview } from "./MQTTOverview";
import { MQTTMessageList } from "./MQTTMessageList";

const persistedMqttSessionIds = new Set<string>();
...
function collectTopLevelJsonKeysFromTopic(topic: string, messages: MQTTMessage[]): string[] {
...
function appendMessage(messages: MQTTMessage[], nextMessage: MQTTMessage): MQTTMessage[] {
...
export function MQTTEditor({
  ...
  const [topic, setTopic] = useState(mqtt.subscriptions[0]?.topic || "demo/topic");
  const [payload, setPayload] = useState("{}");
  const [retain, setRetain] = useState(false);
  const [subscriptionDraftTopic, setSubscriptionDraftTopic] = useState("");
  const [subscriptionDraftQos, setSubscriptionDraftQos] = useState<0 | 1 | 2>(0);
  ...
  const previousSubscriptionsRef = useRef<MQTTSubscription[]>(mqtt.subscriptions);
  ...
  }, [mqtt.id, mqtt.url, mqtt.subscriptions]);
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      unlistenMessage = await listen<MqttIncomingMessage>(
        `mqtt://message/${mqtt.id}`,
        (event) => {
  ...
  }, [mqtt.id, onUpdate, pushSystemMessage]);
  ...
  useEffect(() => {
    ...
    const prevMap = new Map(previousActive.map((item) => [item.topic, item.qos]));
    const currentMap = new Map(currentActive.map((item) => [item.topic, item.qos]));
  ...
  }, [isConnected, isConnecting, mqtt.cleanSession, mqtt.clientId, mqtt.id, mqtt.keepAliveSecs, mqtt.password, mqtt.subscriptions, mqtt.username, pushSystemMessage, resolve, url]);
  ...
  const publishQos = useMemo((): 0 | 1 | 2 => {
    const t = topic.trim();
    if (!t) return 0;
    const sub = mqtt.subscriptions.find((s) => s.topic === t && s.enabled !== false);
  ...
  }, [isConnected, isSending, mqtt.id, onUpdate, payload, publishQos, pushSystemMessage, resolve, retain, topic]);
  ...
  const addSubscription = useCallback(() => {
    ...
    if (!nextTopic) return;
    onUpdate((prev) => {
      const existing = prev.subscriptions.find((item) => item.topic === nextTopic);
      if (existing) {
        return {
          ...prev,
          subscriptions: prev.subscriptions.map((item) =>
            item.id === existing.id ? { ...item, qos: subscriptionDraftQos } : item,
          ),
        };
      }
      return {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex gap-4 border-b border-text/15 p-4">
        <div className="flex flex-1 items-center overflow-hidden rounded-lg bg-inputbox">
          <span className="flex items-center px-4 py-2.5 text-orange-300">
            <SiMqtt size={14} className="text-orange-300" aria-hidden />
          </span>
        ...
        {isConnected ? (
          <button
            onClick={disconnect}
            className="cursor-pointer rounded-full bg-red px-6 py-2 font-semibold text-background transition-all hover:bg-red/90"
          >
            Disconnect
          </button>
        ) : (
          ...
          >
            <button
              ...
              className="cursor-pointer rounded-full bg-accent px-6 py-2 font-semibold text-background transition-all hover:bg-accent/90 disabled:cursor-not-allowed disabled:opacity-50"
            >
      ...
      <div ref={splitContainerRef} className="flex min-h-0 flex-1 overflow-hidden">
        <div
          ...
          style={{ width: activeTab === "overview" ? "100%" : `${splitPercent}%` }}
        >
          <div className="flex items-center gap-1 py-2 shrink-0">
            {(["overview", "publish", "topics", "connection"] as const).map((tab) => (
              <button
                key={tab}
                type="button"
                ...
                <div className="border-b border-white/5 px-4 py-4">
                  <div className={sectionClass}>
                    <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-3">
                      <div>
                        <label className="mb-2 block text-xs text-white/60">Topic</label>
                ...
                <div className="absolute bottom-4 right-4 z-20 inline-flex w-fit h-fit">
                  ...
                  >
                    <button
                      ...
                      disabled={!isConnected || !topic.trim() || !payload.trim() || isSending}
                  ...
                  <div className={sectionClass}>
                    ...
                    <div className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2">
                      ...
                      <div className="relative shrink-0 justify-self-start">
                        <button
                          type="button"
                          onClick={() =>
                            setOpenQosMenuId((prev) => (prev === "draft" ? null : "draft"))
                          }
                        ...
                        >
                          <span className="font-mono text-xs text-white/70">QoS {subscriptionDraftQos}</span>
                          <TbChevronDown size={14} className="shrink-0 text-white/35" />
                        </button>
                        {openQosMenuId === "draft" && (
                          <Dropdown
                            width="min-w-[120px]"
                    ...
                    {mqtt.subscriptions.length === 0 ? (
                      <div className="rounded-lg border border-dashed border-white/8 bg-white/[0.02] px-4 py-5 text-sm text-white/35">
                        No topics yet. Add one above to start listening for broker traffic.
                      </div>
                    ) : (
                      <div className="overflow-hidden rounded-lg border border-white/6 bg-background/25">
                        <div className="grid grid-cols-[minmax(0,1fr)_auto_auto_auto] items-center gap-2 border-b border-white/6 px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-white/35">
                          ...
                          <span className="justify-self-center" title="Subscribe">
                            On
                          </span>
                          ...
                          >
                            ...
                            <div className="relative shrink-0 justify-self-start">
                              <button
                                ...
                                className="inline-flex h-10 w-fit items-center gap-1 rounded-lg border border-white/8 bg-inputbox px-2 text-sm text-white transition-colors hover:border-white/15"
                              >
                                <span className="font-mono text-xs text-white/70">QoS {subscription.qos}</span>
                                <TbChevronDown size={14} className="shrink-0 text-white/35" />
                              </button>
            ...
            {activeTab === "connection" && (
              <div className="space-y-4 p-4">
                <section className={sectionClass}>
                  <div>
                    <label className="mb-2 block text-xs text-white/60">Client ID</label>
                    <input
                      type="text"
                      value={mqtt.clientId}
                      onChange={(e) =>
                        onUpdate((prev) => ({ ...prev, clientId: e.target.value }))
                      }
                      placeholder="Leave blank to auto-generate"
                      className={fieldClass}
                    />
                    <p className="mt-2 text-xs text-white/35">
                      MQTT clients typically need a unique client ID. If you leave this empty,
                      Mandy will generate one at connect time.
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="mb-2 block text-xs text-white/60">Username</label>
                      <input
                        type="text"
                        value={mqtt.username || ""}
                        onChange={(e) =>
                          onUpdate((prev) => ({ ...prev, username: e.target.value }))
                        }
                        className={fieldClass}
                      />
                    </div>
                    <div>
                      <label className="mb-2 block text-xs text-white/60">Password</label>
                      <input
                        type="password"
                        value={mqtt.password || ""}
                        onChange={(e) =>
                          onUpdate((prev) => ({ ...prev, password: e.target.value }))
                        }
                        className={fieldClass}
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="mb-2 block text-xs text-white/60">Keep Alive (secs)</label>
                      <input
                        type="number"
                        min={1}
                        ...
                        onChange={(e) =>
                          onUpdate((prev) => ({
                            ...prev,
                            keepAliveSecs: Math.max(1, Number(e.target.value) || 30),
                          }))
                        }
                        className={fieldClass}
                      />
                    ...
                    <div className="flex items-end">
                      <label className="inline-flex h-10 items-center gap-2 text-sm font-medium text-white/72">
                        <Checkbox
                          ...
                          onChange={(checked) =>
                            onUpdate((prev) => ({ ...prev, cleanSession: checked }))
                          }
                        />
                        Clean session
                      </label>
