...
const METHOD_COLORS: Record<string, { bg: string; text: string; bgHover: string }> = {
    GET: { bg: "bg-green-500/10", text: "text-green-400", bgHover: "hover:bg-green-500/20" },
    POST: { bg: "bg-blue-500/10", text: "text-blue-400", bgHover: "hover:bg-blue-500/20" },
    PUT: { bg: "bg-yellow-500/10", text: "text-yellow-400", bgHover: "hover:bg-yellow-500/20" },
    PATCH: { bg: "bg-purple-500/10", text: "text-purple-400", bgHover: "hover:bg-purple-500/20" },
    DELETE: { bg: "bg-red-500/10", text: "text-red-400", bgHover: "hover:bg-red-500/20" },
    HEAD: { bg: "bg-gray-500/10", text: "text-gray-400", bgHover: "hover:bg-gray-500/20" },
    OPTIONS: { bg: "bg-cyan-500/10", text: "text-cyan-400", bgHover: "hover:bg-cyan-500/20" },
};
...
interface ProjectOverviewProps {
    ...
    onUpdateEnvVar?: (envId: string, varId: string, key: string, value: string, enabled: boolean) => void;
...
function RequestDetails({
    ...
    if (hasBody && typeof body === "object" && "Raw" in body && body.Raw.content_type?.includes("json")) {
        try {
    ...
    return (
        <div className={`group/card flex flex-col bg-white/[0.02] border-x border-b first:border-t border-white/5 ${borderRadiusClasses} overflow-hidden hover:bg-white/[0.04] transition-all relative`}>
            <div className={`absolute left-0 top-0 bottom-0 w-1 ${colors.bg.replace("/10", "/40")} `} />

            <button
                onClick={() => setExpanded(!expanded)}
                className="w-full flex items-stretch cursor-pointer text-left overflow-hidden h-10"
            >
                <div className={`flex items-center justify-start px-4 ${colors.bg} ${colors.text} shrink-0 min-w-[69px] transition-colors`}>
                ...
                <div className="flex-1 flex items-center justify-between px-4 min-w-0">
                    ...
                    <div className="flex items-center gap-3 shrink-0 ml-4">
                        ...
                        {expanded ? (
                            <HiChevronDown size={14} className="text-white/20 group-hover/card:text-white/50 transition-colors" />
                        ) : (
                            <HiChevronRight size={14} className="text-white/20 group-hover/card:text-white/50 transition-colors" />
                        )}
                    </div>
                </div>
            </button>
...
function FolderSection({
    ...
    return (
        <div className="w-full">
            ...
            >
                {isExpanded ? (
                    <HiChevronDown size={14} className="text-white/20 group-hover:text-white/40 shrink-0" />
                ) : (
                    <HiChevronRight size={14} className="text-white/20 group-hover:text-white/40 shrink-0" />
                )}
            ...
            {isExpanded && (
                <div className={`flex flex-col gap-2 ${depth > 0 ? "ml-3 border-l border-white/10 pl-4 mb-1 mt-1" : ""}`}>
...
export function ProjectOverview({
    ...
    const [expandedIds, setExpandedIds] = useState<Set<string>>(new Set(getAllFolderIds(project.root)));
    ...
    const [expandedEnvId, setExpandedEnvId] = useState<string | null>(project.activeEnvironmentId);
    const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
    const iconButtonRef = useRef<HTMLButtonElement>(null);

    const allRequests = useMemo(() => collectRequests(project.root), [project.root]);
    const selectedLanguage = useProjectStore(state => state.selectedLanguage);
    const setSelectedLanguage = useProjectStore(state => state.setSelectedLanguage);
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden">
            <div>
                <div className="flex items-center gap-4 border-b border-white/5 pb-3 p-4" data-tauri-drag-region>
                    <button
                        ref={iconButtonRef}
                        ...
                        style={{
                            ...
                            backgroundColor: project.iconColor ? hexToRgba(project.iconColor, 0.05) : "rgba(255, 255, 255, 0.05)"
                        }}
                    >
            ...
            <div className="flex-1 overflow-y-auto p-6">
                        ...
                        <div className="flex items-start justify-between">
                            ...
                            <div className="flex flex-col items-end shrink-0 relative">
                                ...
                                >
                                    ...
                                    <HiChevronDown size={14} className="text-white/20 group-hover:text-white/40 transition-colors mt-0.5" />
                                </button>
                                {showLangDropdown && (
                                    <Dropdown
                ...
                {activeTab === "configuration" && (
                    <div className="space-y-6 max-w-xl">
                        <div>
                            <label className="block text-xs font-medium text-white/50 mb-2">Base URL</label>
                            <input
                                type="text"
                                value={baseUrl}
                        ...
                        <div className="pt-6 border-t border-white/5">
                            ...
                            <p className="text-xs text-white/30 mb-4">Once you delete a project, there is no going back. Please be certain.</p>
                            <button
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            {project.environments.map((env) => (
                                <div key={env.id} className={`border rounded-xl overflow-hidden transition-all ${env.id === project.activeEnvironmentId ? "border-accent/30" : "border-white/10"}`}>
                                    <div
                                    ...
                                    >
                                        <span className={`text-[10px] w-4 ${env.id === project.activeEnvironmentId ? "text-accent" : "text-white/30"}`}>
                                            {expandedEnvId === env.id ? "▼" : "▶"}
                                        </span>

                                        <input
                                            type="text"
                                            value={editingEnvId === env.id ? editingEnvName : env.name}
                                            ...
                                            onKeyDown={(e) => e.key === "Enter" && handleEnvRename(env.id)}
                                        ...
                                        <span className={`text-xs ml-1 mr-auto font-medium ${env.id === project.activeEnvironmentId ? "text-accent/60" : "text-white/20"}`}>
