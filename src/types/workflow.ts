...
/** Runs when a workflow executes a request node (typically REST via `sendRequest`). */
export type WorkflowExecuteRequestHandler = (
	requestId: string,
	context: WorkflowExecutionContext,
	overrides?: RequestOverrides,
) => Promise<NodeOutput>;

/** Optional flash metadata stored on edges during workflow run. */
export type WorkflowEdgeFlashData = {
	__flash?: number | null;
	__flashColor?: string | null;
	__flashMs?: number | null;
};

/** Ephemeral UI fields on nodes (not required to persist). */
export type WorkflowNodeFlashData = {
	__flashCount?: number;
};
